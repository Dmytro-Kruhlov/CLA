...
def search_note_by_name(name: str) -> None:
    note_files = [
        f for f in os.listdir(NOTES_FOLDER) if f.endswith(".md") and name in f
    ]
...
def note_book() -> str:
    ...
    while True:
        command = input('\nEnter valid command or enter "exit" to leave>>> ')
        parts = command.split()
        ...
        if action == "help":
            print(
                "[i] Commands: 'add note <name> <tags>', 'delete note <name>', 'edit note <name>', 'edit tags "
                "<name>', 'show all', 'search note <name>', 'search tag <tag>', exit"
            )
